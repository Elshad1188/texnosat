      ...
      <main className="container mx-auto max-w-3xl px-4 py-6 pb-24">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-amber-400 via-orange-500 to-pink-500 p-6 sm:p-8 text-white shadow-xl mb-6">
          ...
          <div className="relative">
            ...
            <p className="text-[11px] text-white/80 mt-2 text-center">
              + {Number(settings?.bonus_balance_amount || 1)} AZN balansa bonus (ödənişli qoşulmada). Qaliblər eyni mükafatı alır — qoşulma üsulundan asılı deyil.
            </p>
          </div>
        </div>

        {/* Referral / share link card — always visible */}
        <Card className="p-4 mb-6 bg-gradient-to-br from-amber-500/10 to-orange-500/10 border-amber-500/30">
          <h3 className="font-bold mb-1 flex items-center gap-2 text-sm">
            ...
            {myParticipation ? "Sənin yarışma dəvət linkin" : "Yarışmanı paylaş və pulsuz qoşulma şansı qazan"}
          </h3>
          <p className="text-[11px] text-muted-foreground mb-3">
            {myParticipation
              ? "Bu link yalnız bu yarışma üçündür — hər qeydiyyat sənə 1 dəvət sayılır."
              : `Linkini paylaş — son ${freeWindowHours} saatda ${freeRequired} nəfər qeydiyyatdan keçsə, yarışmaya PULSUZ qoşula bilərsən.`}
          </p>
          {(() => {
            const code = myParticipation?.referral_code || user?.id?.slice(0, 8).toUpperCase();
            const link = myParticipation
              ? `${window.location.origin}/r/${myParticipation.referral_code}`
              : user
                ? `${window.location.origin}/auth?ref=${code}`
                : `${window.location.origin}/contest`;
