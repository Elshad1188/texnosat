  ...
  const sendMessage = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("chatbot", {
        body: { message: userMsg.content, history: messages },
      });
            ...
            {messages.length === 0 && (
              <div className="flex flex-col items-center py-8 text-center">
                ...
                <div className="mt-4 flex flex-wrap gap-2 justify-center">
                  {["Elan necə yerləşdirim?", "Mağaza necə açım?", "Premium nədir?"].map((q) => (
                    <button
                      key={q}
                      className="rounded-full border border-border px-3 py-1.5 text-[11px] text-foreground hover:bg-muted transition-colors"
                      onClick={() => { setInput(q); }}
                    >
                      {q}
                    </button>
