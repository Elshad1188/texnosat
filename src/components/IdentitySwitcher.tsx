...
const IdentitySwitcher = forwardRef<HTMLDivElement, IdentitySwitcherProps>(
  ({ selectedStoreId, onSelect, compact = false }, ref) => {
    ...
    if (!user || approvedStores.length === 0) {
      return <span ref={ref as any} style={{ display: "none" }} aria-hidden />;
    }
      ...
      <div ref={ref} className="inline-block">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              type="button"
              className={`flex items-center gap-2 rounded-lg border border-border/50 bg-muted/30 px-3 py-1.5 text-xs font-medium text-foreground hover:bg-muted/60 transition-colors ${
                compact ? "py-1 px-2" : ""
              }`}
            >
...
IdentitySwitcher.displayName = "IdentitySwitcher";
